...
static int checka(const char *hostname)
{
	RFC1035_ADDR *addrs;
	unsigned        naddrs, n;
	int rc=rfc1035_a(&rfc1035_default_resolver, hostname, &addrs,
			 &naddrs);
...
static int getsender(Input &input, struct rw_transport *module)
{
	    ...
	    getenv("RELAYCLIENT") == NULL)
	{
		...
		*/
		...
		if (ipcmp(frominfo.helohost.c_str(), tcpremoteip)) switch (rfc1035_mxlist_create(&rfc1035_default_resolver,
												 frominfo.helohost.c_str(), &mxlist))
		{
		case RFC1035_MX_OK:
			...
			{
				int rc=checka(frominfo.helohost.c_str());
			...
			std::cout << harderr << " HELO " << frominfo.helohost
		...
		case RFC1035_MX_BADDNS:
			std::cout << "517 RFC 1035 violation: recursive CNAME records for " << frominfo.helohost << "." << std::endl;
			return 1;
		default:
			std::cout << "417 DNS lookup on HELO " << frominfo.helohost <<
...
static const char *trackaddress(const char *p)
{
	...
	{
		results=track_find(p, &timestamp);
...
char	*sender=rfc822_gettok(addresst);
	...
	else
	{
		...
		{
			static char accept[]="BOFHBADMIME=accept";
			putenv(accept);
		}
...
struct rw_transport *modulep=rw_search_transport(module);
	...
	{
		alarm_timeout=0; /* No timeout for local mail */
	}
	else
	{
		alarm_timeout=1800;
